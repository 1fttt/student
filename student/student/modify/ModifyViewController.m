...
- (void)pressBack {
    ...
    if(_nameTextField.text.length == 0 || _classTextField.text.length == 0 || _numTextField.text.length == 0 || _scoreTextField.text.length == 0) {
        ...
        for(int i = 0; i < _numArr.count; i++) {
            if([_numArr[i] isEqualToString:_numTextField.text] && (![ _numstr isEqualToString:_numTextField.text])) {
                t = 1;
                break;
            }
        }
        if(t == 1) {
            ...
            _numArr[k] = [NSString stringWithString:_numTextField.text];
            _scoreArr[k] = [NSString stringWithString:_scoreTextField.text];
            
            [self.delegate modifyContent:_nameArr andClass:_classArr andNum:_numArr andScore:_scoreArr];
